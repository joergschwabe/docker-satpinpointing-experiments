 ...
 EL2MCS: Axiom Pinpointing in EL+
-----------------------------------

The distribution folder contains the following directories/files:

1) el2mcs/
  - Binary executable of EL2MCS (Linux-64bit)
...
+ el2mcs usage:
	./el2mcs [<option>*] <path_to_wcnf_file>
	where <option> includes:
		-T TTT: time limit (in seconds) [default: T 3600]
                -st Print statistics
  
        and <path_to_wcnf_file> file contains the partial MaxSAT encoding described in [1].


 EL2MCS first prints on stdout the set of all Minimal Correction Subsets (MCSes) of the wcnf formula provided as input, each one of them in a line starting by "c MCS ". MCSes correspond to minimal repairs of the ontology w.r.t. a given inferred subsumption relation. 

 Then, EL2MCS computes the Minimal Unsatisfiable Subformulas of the wcnf formula by applying minimal hitting set enumeration of the first set. Each MUS corresponds to a minimal axiom set (minA) of the ontology w.r.t a given subsumption relation. These are printed on stdout, each in a line starting by "c MUS: ".

Both MCSes and MUSes are sequences of numbers, where each number corresponds to an identifier of either an original GCI or an original RI.
...
STEP 3. Create a query file with axiom pinpointing queries before generating wcnf instances
  ...
  - The file must contain one index per line, corresponding to a subsumption relation
    in <name>.zzz. This will allow for generating one wcnf instance per query.
 ...
 --------
  ...
  A) Without any optimizations:
     ...
     As a result, the folder med/nop is created, containing 2 files:
       - med.22.wcnf: wcnf file to be fed to el2mcs, corresponding to query
          22 (implies Endocardium (some contIn Heart) )

       - med.26.wcnf: wcnf file to be fed to el2mcs, corresponding to query
          26 (implies Endocarditis HeartDisease )
   ...
   B) With the COI optimization
     ...
     As a result, the folder med/coi is created, containing:
       - med.22.coi.wcnf: wcnf file to be fed to el2mcs, corresponding to query 22
       - med.22.module.axioms: COI module for query 22 (axioms that may take part in some justifications)
       - med.26.coi.wcnf: wcnf file to be fed to el2mcs, corresponding to query 26
   ...
   3) With the x2 optimization
      ...
      ./scripts/create-wcnf med med/class med/queries.q med/x2 tools/ x2
       ...
       - med.22.x2.wcnf: wcnf file to be fed to el2mcs, corresponding to query 22
       - med.22.krss: file with the subontology for query 22
       - med.22.x2.map: list of GCIs and RIs that may be involved in some justifications with their
                        new and original indices (in med/class/med.zzz.gci and med/class/med.zzz.ri)
        ...
        EL2MCS will print MCSes and MUSes as lists of indices from the first column.
          

       - med.26.x2.wcnf: wcnf file to be fed to el2mcs, corresponding to query 26
       - med.26.module.axioms: COI module for query 26 (axioms that may take part in some justifications)
  
  
  Note: EL2MCS performs best with the x2 optimization.

  Note: For queries on axioms with new primitive concept names, coi and x2 may not work properly.
        Thus, it is recommended not to perform any optimizations in these cases.
        These axioms are the result of the normalization process, and so axiom pinpointing
        is expected to be very efficient even without optimizations.





++++++++++++++++++++++++ RUNNING THE EXAMPLE WCNF WITH EL2MCS ++++++++++++++++++++++++++++++++

  To finish the example, we show how to solve the axiom pinpointing wcnf instance with el2mcs
      22 (implies Endocardium (some contIn Heart) )

  With both no optimizations and coi we interpret the output in the same way:

   ./el2mcs med/nop/med.22.wcnf
    
    (or ./el2mcs med/coi/med.22.coi.wcnf)

    ******************** el2mcs output ******************************
     ...
     c (MCSes/Repairs/Diagnosis) axioms:
     c MCS: 6 
     c MCS: 4 
     c MCS: 3 
     c (MUSes/MinAs/Conflicting Sets) axioms:
     c MUS: 6 4 3 
     c Number of MUSes: 1
     c Number of MCSes: 3
     ...
    ******************************************************************
    ...
    The MUS is {6,4,3}, which looking at the classification files med/class/med.zzz.gci and
     med/class/med.zzz.gci, corresponds to the axioms:
...
[1] M. Fareed Arif, Carlos Mencia, and Joao Marques-Silva, "Efficient Axiom Pinpointing with EL2MCS" - KI 2015. 
